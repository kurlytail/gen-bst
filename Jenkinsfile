...
pipeline {
    ...
    stages {
        stage('Prepare env') {
            ...
            steps {
                script {
                    loadLibrary()
                    env['NPM_VERSION_NUMBER'] = getNpmVersion 'kurlytail/gen-bst/master', params.BUILD_VERSION_PREFIX, params.BUILDS_OFFSET
    ...
    post {
        always {
            slackSend message: "gen-bst build ${env.NPM_VERSION_NUMBER} - Status ${currentBuild.result} - ${env.BUILD_URL}"
        }
    }
}
